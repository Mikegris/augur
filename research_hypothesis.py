...
def _call_openai(prompt: str, api_key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        cap_check = getattr(ai_summarizer, "_cap_exceeded", None)
        if cap_check is None:
            # The private symbol was renamed/removed — surface it instead of
            # silently letting calls proceed un-capped (shared budget bypass).
            log.warning("ai_summarizer._cap_exceeded missing; daily AI cap NOT "
                        "enforced for hypothesis calls")
        elif cap_check():
...
def score_hypothesis(hypothesis_id: int) -> Dict[str, Any]:
    ...
    # NOTE: horizon_days here is deliberately CALENDAR days, not trading days.
    # Unlike research_tracker.score_due_forecasts (which converts trading-day
    # signal horizons via ceil(h*7/5)), a hypothesis is a human-facing
    # "verify in N days" wall-clock window generated by the LLM prompt, so the
    # due date is a straight calendar offset. Do NOT add a trading-day
    # conversion here — it would resolve hypotheses against the wrong window.
    ...
    # A non-positive entry price can never resolve: zero divides by zero, and a
    # negative (corrupt) price flips the sign of realized_pct, inverting UP/DOWN.
    # Treat any non-positive (and NaN) price like a missing price and invalidate
    # instead of OPEN-forever or scoring a nonsensical inverted return.
    try:
        issued_ok = float(issued) > 0
    except (TypeError, ValueError):
        issued_ok = False
    if not issued_ok:
        with closing(_conn()) as c:
